...
const CatalogsList = () => {
    ...
    const loadData = async () => {
        try {
            ...
            // Como la API devuelve un array directo:
    ...
    return (
        <Layout>
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-gray-800">Juegos</h1>
                <button
            ...
            <div className="bg-white shadow-md rounded-lg overflow-hidden">
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            <th className="text-xs font-medium text-gray-500 uppercase tracking-wider">Juego</th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Plataforma</th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Lanzamiento</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {catalogs.length === 0 ? (
                            ...
                            catalogs.map((item) => (
                                <tr
                                    key={item.id}
                                    className={`hover:bg-gray-50 transition-colors ${recentlyUpdated === item.id
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-900">{item.plataform}</td>
                                    <td className="px-6 py-4 text-sm text-gray-500">{formatDate(item.release_date)}</td>
                                    <td className="px-6 py-4 text-sm text-gray-900">{formatCurrency(item.cost)}</td>
                                    ...
                                    <td className="px-6 py-4">
                                        <span
                                            className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${item.active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                                }`}
                                        >
